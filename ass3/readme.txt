...
The purpose with `myfree` is to free the memory block. In order to avoid adjacent free blocks the function checks adjacent blocks every time the function called, and if they are free too, so `myfree`  coalesces them.. 
